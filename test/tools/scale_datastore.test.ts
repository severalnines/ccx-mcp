...
describe("scale_datastore", () => {
  it("sends PATCH with new_instance_size", async () => {
    ...
    await patch(`/prov/api/v2/cluster/${DS_UUID}`, { new_instance_size: "medium" });

    expect(capturedBody).toEqual({ new_instance_size: "medium" });
  });

  it("sends PATCH with new_volume_size", async () => {
    let capturedBody: Record<string, unknown> = {};
    ...
    const { patch } = await import("../../src/client.js");
    await patch(`/prov/api/v2/cluster/${DS_UUID}`, { new_volume_size: 200 });

    expect(capturedBody).toEqual({ new_volume_size: 200 });
  });

  it("sends PATCH with both instance and volume", async () => {
    let capturedBody: Record<string, unknown> = {};
  ...
  it("surfaces 400 for invalid instance size", async () => {
    mswServer.use(
      http.patch(`https://test.ccx.dev/api/prov/api/v2/cluster/${DS_UUID}`, () => {
        return HttpResponse.json({ error: "invalid instance size" }, { status: 400 });
      }),
    );
    ...
    try {
      await patch(`/prov/api/v2/cluster/${DS_UUID}`, { new_instance_size: "nonexistent" });
  ...
  it("surfaces 409 when job already running", async () => {
    mswServer.use(
      http.patch(`https://test.ccx.dev/api/prov/api/v2/cluster/${DS_UUID}`, () => {
        return HttpResponse.json({ error: "job already in progress" }, { status: 409 });
      }),
    );
  ...
  it("surfaces 404 for nonexistent datastore", async () => {
    ...
    try {
      await patch("/prov/api/v2/cluster/nonexistent", { new_instance_size: "medium" });
      expect.unreachable("Should have thrown");
    } catch (e) {
      expect(e).toBeInstanceOf(ApiError);
  ...
  it("sends correct auth headers", async () => {
    ...
    const { patch } = await import("../../src/client.js");
    await patch(`/prov/api/v2/cluster/${DS_UUID}`, { new_instance_size: "small" });
